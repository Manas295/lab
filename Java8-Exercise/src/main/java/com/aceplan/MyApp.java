...
public class MyApp {
	public static void main(String[] args) {
		...
		/*
		 * Q1 Write a program using streams to remove all odd numbers and then square
		 * the result and print the sum of all squares. Change the code to not use the
		 * first 3 even numbers
		 */
		System.out.println("----------------Q1-----------------");
		System.out.println("Sum : "
				+ IntStream.range(1, 10).filter(i -> i % 2 != 0).skip(3).map(i -> i * i).reduce(0, Integer::sum));
		...
		/*
		 * Q3 Create a Movie class which has ratings. From a Set of movies print the Top
		 * rated movies, average and the lowest rating movies.
		 * 
		 */
		...
		/*
		 * Q5 If I have list of movies with ratings and year of release. I want to first
		 * sort them based on ratings and then year of release
		 * 
		 */
		System.out.println("----------------Q5-----------------");
		movieList.stream()
				.sorted(Comparator.comparing(Movie::getRating).thenComparing(Comparator.comparing(Movie::getYear)))
				.forEach(System.out::println);
		System.out.println("----------------------");

		/*
		 * I have a Developer class. Each developer is can have multiple skills like
		 * Java, Java script, database, .Net and so on. From a list of Developers find
		 * out the unique skills available in the company
		 * 
		 */
		...
		long count = map.flatMap(Arrays::stream).filter(a -> a.equalsIgnoreCase("e")).count();
		System.out.println("Count  :" + count);

		/*
		 * I have a stream of strings and I want to collect them in a map with key as
		 * length and value as a Set of those words
		 */
		System.out.println("----------------Q8-----------------");

		HashMap<Integer, Set<String>> mapOfString = Arrays.asList("Ken", "Jeff", "Ellen").stream()
				.collect(Collectors.groupingBy(String::length, HashMap::new, Collectors.toSet()));
		mapOfString.entrySet().stream()
